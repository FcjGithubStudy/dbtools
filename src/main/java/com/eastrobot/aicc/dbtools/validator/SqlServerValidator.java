...
/**
 * 模块名称: SqlServer数据库校验器
 * 模块描述: 校验SqlServer数据库中表结构完整性,缺表的自动添加表(并插入内置数据);缺字段的增加字段(但不会再插入内置数据);缺索引的自动加非主键索引
 *
 * @author xinchi.wang
...
public class SqlServerValidator {
     ...
     * 表名-列名专属转义符
     */
    private static final String DB_ESCAPE_CHARACTER = DbToolsConstants.DB_ESCAPE_CHARACTER_SQLSERVER;
    /**
     * 【表名-->建表语句】构成的Map
     */
    ...
    private static final Log log = LogFactory.getLog(SqlServerValidator.class);
     ...
     */
    public SqlServerValidator(DbToolsConfig dbToolsConfig) {
    ...
    private void classifySqls() throws IOException {
        ...
        Resource resource = new ClassPathResource("sqlserver/sqlserver_tenant.sql");
